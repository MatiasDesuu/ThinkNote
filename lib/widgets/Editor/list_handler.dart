...
class ListHandler extends StatelessWidget {
  ...
  List<TextSpan> _buildTextSpansWithLists(BuildContext context) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (listItem != null && listItem.type == ListType.checkbox) {
        ...
        final listSpans = _buildSpansWithLinks(context, listItem.formattedText, textStyle.copyWith(
          fontWeight: FontWeight.normal, // Normal weight to match regular text
        ));
  ...
  TextSpan _buildCheckboxSpan(BuildContext context, ListItem listItem, int lineIndex) {
    ...
    children.add(TextSpan(
      ...
      style: textStyle.copyWith(
        ...
        color: listItem.isChecked ? Colors.grey : textStyle.color, // Subtle color for checkbox
      ),
  ...
  List<TextSpan> _buildSpansWithLinks(BuildContext context, String text, TextStyle style) {
    final List<TextSpan> spans = [];
...
class ListDetector {
  ...
  static ListItem? detectListItem(String line, {int lineNumber = 0}) {
    ...
    // Check for asterisk lists (*)
    final asteriskMatch = _asteriskListRegex.firstMatch(line);
    if (asteriskMatch != null) {
      final marker = asteriskMatch.group(1)!;
      final content = asteriskMatch.group(2)!;
      final indentLevel = _calculateIndentLevel(line);
      
      return ListItem(
        type: ListType.asterisk,
        marker: marker,
        content: content,
        formattedText: '${_getIndentString(indentLevel)}• $content', // Use same bullet symbol for consistency
        indentLevel: indentLevel,
        lineNumber: lineNumber,
      );
    }
  ...
  void _handleTextChange(String value) {
    ...
    if (selection.isValid && selection.isCollapsed) {
      ...
      if (lines.isNotEmpty) {
        ...
        if (currentLine.isEmpty && ListDetector.detectListItem(previousLine) != null) {
          final continuationMarker = ListDetector.getContinuationMarker(previousLine);
          
          if (continuationMarker.isNotEmpty) {
            // Insert the continuation marker
            final newText = value.substring(0, cursorPosition) + 
                           continuationMarker + 
    ...
    for (final item in listItems) {
      switch (item.type) {
        ...
        case ListType.bullet:
          ...
          break;
        case ListType.asterisk:
          stats['asterisk'] = (stats['asterisk'] ?? 0) + 1;
          break;
        case ListType.checkbox:
